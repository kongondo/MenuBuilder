 ...
 *
 * @Modified by Kongondo for the ProcessWire Module MenuBuilder (February 2015) - see @notes in comments
 */
...
(function($) {
	...
	$.fn.asmSelectMB = function(customOptions) {
		...
		var options = {
			...
			listType: 'ol',					// @@note - modified to use tables instead: originally Ordered list 'ol', or unordered list 'ul'
		...
		return this.each(function (index) {
			...
			var $ol; 						// @note: the table (list) that we are manipulating
			...
			function makeSortable() {
				...
				$ol.sortable({
					items: 'li.' + options.listItemClass,//@note: originally <li>
					...
					update: function(e, ui) {
						...
						$(this).children("li").each(function(n) {//@note: originally <li>
			...
			function addListItem(optionId) {
				...
				var $removeLink = $("<a></a>")
					...
					.click(function() { 
						dropListItem($(this).parent('li').attr('rel'));//@note: originally <li>
						return false; 
					}); 
			...
			function dropListItem(optionId, highlightItem) {
				...
				if(options.hideDeleted) {
					...
					$item = $ol.children("li[rel=" + optionId + "]");//@note: originally <li>
			...
			function dropListItemHide($item) {
				...
				if(options.animate && !buildingSelect) {
					...
					$prevItem = $item.prev("li");//@note: originally <li>
